...
colors() is a function with a bunch of keyword arguments
  the test code calls it in various ways and confirms the results are
  as expected.

call_colors() is a functiont hat takes totaly generic arguments
  the tet code calls it in various ways so you can confirm that
  it works as expected.

colors_manual() takes generic arguments, but processes it as if it had
keyword parameters.
  - you'd never write code like this, but it shows what's going on under
    the hood.
...
def colors_manual(*args, **kwargs):
    ...
    #  one reason that dicts preserve order now.
    default_colors = {'fore_color': 'red',
                      'back_color': 'blue',
                      'link_color': 'green',
                      'visited_color': 'cyan',
                      }
